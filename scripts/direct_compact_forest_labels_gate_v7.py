...
replacements = (
    (
        ...
        'WORKFLOW = Path(".github/workflows/direct-compact-forest-labels-v7.yml")',
        "workflow path",
    ),
    (
        'SCRIPT = Path("scripts/direct_compact_forest_labels_gate.py")',
        'SCRIPT = Path("scripts/direct_compact_forest_labels_gate_v7.py")',
        "script path",
    ),
    (
    ...
    (
        ...
        "FOREST_TYPE_MARKER = '''#[derive(Debug, Clone, PartialEq)]\npub struct ForestGrouping {\n'''\n",
        "forest grouping derive marker",
    ),
)
...
required = (
    "direct_compact_forest_labels_gate_v7.py",
    "direct-compact-forest-labels-v7.yml",
    "FOREST_TYPE_MARKER = '''#[derive(Debug, Clone, PartialEq)]",
    "impl PartialEq<&[usize]> for ForestAggregationLabels",
    "(label as usize) < aggregate_count",
)
