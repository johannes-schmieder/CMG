...
WORKFLOW = Path(".github/workflows/compact-forest-indegree-memory.yml")
SCRIPT = Path("scripts/compact_forest_indegree_memory_gate.py")
RECORD = Path(".ci/performance/compact-forest-indegree-memory-latest.json")
TIMING_RECORD = Path(".ci/performance/compact-forest-indegree-latest.json")
PLAN = Path("PERFORMANCE_PLAN.md")
STATUS = Path("PERFORMANCE_STATUS.md")
HISTORICAL_COMMIT = "fc7e594173e79f86b0220aa64f64d28aa7bef61e"
HISTORICAL_SCRIPT = "scripts/compact_forest_indegree_gate.py"
...
'''
    if historical.count(old_logic) != 1:
        raise RuntimeError("historical decrement transformation changed")
    historical = historical.replace(old_logic, new_logic, 1)
    marker = "baseline_source = SOURCE.read_text()"
    if marker not in historical:
        raise RuntimeError("historical execution marker missing")
    namespace = {"__name__": "compact_indegree_memory_gate_defs"}
...
def sample(binary, arguments, tag):
    time_path = Path(f"/tmp/cmg-compact-indegree-memory-{tag}.time")
    completed = run(
        [
            "/usr/bin/time",
            "-v",
...
def update_documents(result):
    ...
    hierarchy_ratio = result.get("prior_hierarchy_geometric_time_ratio", 1.0)
    checkpoint = f'''### Compact forest-indegree exact-memory checkpoint — 2026-08-23

- The compact `u32` forest-indegree candidate was **{decision}** after exact-memory requalification.
- Validation: `{result.get("validation", "unknown")}`.
- Prior hierarchy timing ratio: `{hierarchy_ratio:.3f}x`.
...
'''
    ...
    heading = "### Compact forest-indegree exact-memory checkpoint — 2026-08-23\n"
    if heading in plan:
    ...
    block = f'''## Compact forest-indegree exact-memory gate
...
'''
    ...
    heading = "## Compact forest-indegree exact-memory gate\n"
...
result = {
    "schema_version": 1,
    "experiment": "compact-forest-indegree-exact-memory-requalification",
...
try:
    ...
    baseline = build(Path("/tmp/cmg-compact-indegree-memory-baseline"))
    ...
    candidate = build(Path("/tmp/cmg-compact-indegree-memory-candidate"))
    specs = (
    ...
    result["prior_split_geometric_time_ratio"] = timing.get(
        "split_geometric_time_ratio", 1.0
    )
    result["prior_hierarchy_geometric_time_ratio"] = timing.get(
        "hierarchy_geometric_time_ratio", 1.0
    )
    result["prior_path_hierarchy_time_ratio"] = timing.get(
        "path_hierarchy_time_ratio", 1.0
    )
    prior_split_rss = [
        case["candidate_over_baseline_peak_rss"]
        for case in timing.get("split_cases", {}).values()
    ]
    result["prior_worst_split_peak_rss_ratio"] = (
        max(prior_split_rss) if prior_split_rss else 1.0
    )
    result["acceptance_limits"] = {
        "prior_split_geometric_time_ratio_max": 0.95,
        "prior_hierarchy_geometric_time_ratio_max": 0.99,
        "prior_path_hierarchy_time_ratio_max": 0.98,
    ...
    result["decision_reason"] = (
        "prior timing gains were confirmed against non-regressive exact hierarchy memory; compact forest indegrees retained"
        if result["accepted"]
...
for key in (
    ...
    "prior_path_hierarchy_time_ratio",
...
run(["git", "add", "-A"])
message = (
    "perf: retain compact forest indegrees after exact memory gate"
    if result.get("accepted", False)
    else "perf: record compact forest-indegree exact-memory result"
)
...
for _ in range(5):
    ...
    raise RuntimeError("failed to push compact forest-indegree memory decision")
