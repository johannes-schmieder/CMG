...
REPO = "CMG"
WORKFLOW_NAME = "conductance-front-list.yml"
DECISION = Path(".ci/performance/conductance-front-list-latest.json")
STATUS = Path(".ci/performance/conductance-front-list-run-status.json")
MONITOR_WORKFLOW = Path(".github/workflows/monitor-conductance-front-gate.yml")
MONITOR_SCRIPT = Path("scripts/monitor_conductance_front_gate.py")
...
def request(path):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cmg-conductance-gate-monitor",
        },
    )
...
record = {
    "schema_version": 1,
    "monitor": "conductance-front-list",
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record conductance-front gate status"])
for _ in range(8):
    ...
    raise RuntimeError("failed to push conductance-front monitor result")
