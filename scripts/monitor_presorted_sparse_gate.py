...
REPO = "CMG"
GATE_SHA = "10d765d8ef71b7715ec49028e9033f1d4e2bdc53"
WORKFLOW_NAME = "presorted-sparse-constructor.yml"
DECISION = Path(".ci/performance/presorted-sparse-constructor-latest.json")
STATUS = Path(".ci/performance/presorted-sparse-constructor-run-status.json")
WORKFLOW = Path(".github/workflows/monitor-presorted-sparse-gate.yml")
SCRIPT = Path("scripts/monitor_presorted_sparse_gate.py")
...
def request(path):
    request_object = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cmg-presorted-sparse-monitor",
        },
    )
...
record = {
    "schema_version": 1,
    "monitor": "presorted-sparse-constructor",
    "gate_sha": GATE_SHA,
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record presorted sparse gate status"])
for _ in range(12):
    ...
    raise RuntimeError("failed to push presorted sparse monitor result")
