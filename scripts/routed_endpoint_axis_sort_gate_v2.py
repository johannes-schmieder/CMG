...
text = text.replace(
    ...
    "routed_endpoint_axis_sort_gate_v2.py",
)
text = text.replace(
    "routed-endpoint-axis-sort.yml",
    "routed-endpoint-axis-sort-v2.yml",
)
...
SCRIPT.unlink(missing_ok=True)
Path(".github/workflows/routed-endpoint-axis-sort.yml").unlink(missing_ok=True)
Path("scripts/routed_endpoint_axis_sort_gate.py").unlink(missing_ok=True)
'''
...
for marker in required:
    if marker not in text:
        raise SystemExit(f"repaired routed endpoint gate missing marker: {marker}")
