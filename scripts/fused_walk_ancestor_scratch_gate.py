...
TEMP_BENCH = Path("src/bin/fused-walk-ancestor-scratch-gate.rs")
WORKFLOW = Path(".github/workflows/fused-walk-ancestor-scratch.yml")
SCRIPT = Path("scripts/fused_walk_ancestor_scratch_gate.py")
RECORD = Path(".ci/performance/fused-walk-ancestor-scratch-latest.json")
...
'''
NEW_SCRATCH = '''    let mut walk = Vec::<ForestWalkEntry>::new();
'''
OLD_INIT = '''            walk.clear();
            walk.push(current);
            new_ancestors.clear();
...
NEW_PUSH = '''                k += 1;
                ...
                walk.push(ForestWalkEntry {
                    vertex: current,
                    ancestor_prefix: ancestors_in_path,
                });
'''
...
#[cfg(test)]
mod fused_walk_ancestor_scratch_tests {
    use super::ForestWalkEntry;

    #[test]
    fn fused_entry_preserves_vertex_and_prefix() {
        let entry = ForestWalkEntry {
            vertex: 17,
            ancestor_prefix: 4,
        };
        assert_eq!(entry.vertex, 17);
        assert_eq!(entry.ancestor_prefix, 4);
        assert_eq!(std::mem::size_of::<ForestWalkEntry>(), 16);
    }
}
'''
...
def apply_candidate(source):
    ...
    candidate = source.replace(INSERT_MARKER, ENTRY_TYPE + INSERT_MARKER, 1)
    replacements = (
    ...
    if "mod fused_walk_ancestor_scratch_tests" not in candidate:
...
def build(target):
    ...
    run(["cargo", "build", "--release", "--bin", "fused-walk-ancestor-scratch-gate"], env=env)
    run(
        [
            "cargo",
            "build",
    ...
    return {
        "split": release / "fused-walk-ancestor-scratch-gate",
...
def sample(binary, arguments, tag):
    time_path = Path(f"/tmp/cmg-fused-walk-{tag}.time")
...
def update_documents(result):
    ...
    checkpoint = f'''### Fused walk/ancestor scratch checkpoint — 2026-08-24

- Replacing parallel walk and ancestor-prefix vectors with one cache-local entry vector was **{decision}**.
...
'''
    ...
    if "### Fused walk/ancestor scratch checkpoint — 2026-08-24\n" not in plan:
    ...
    block = f'''## Fused walk/ancestor scratch gate
...
'''
    ...
    heading = "## Fused walk/ancestor scratch gate\n"
...
try:
    run(["cargo", "fmt", "--all"])
    baseline = build(Path("/tmp/cmg-fused-walk-baseline"))
    ...
    candidate = build(Path("/tmp/cmg-fused-walk-candidate"))
    specs = (
    ...
    result["acceptance_limits"] = {
        ...
        "path_hierarchy_time_ratio_max": 1.01,
    ...
    result["accepted"] = (
        ...
        and result["path_hierarchy_time_ratio"] <= 1.01
...
message = (
    ...
    else "perf: record fused walk-ancestor scratch experiment"
)
...
for _ in range(5):
    ...
    raise RuntimeError("failed to push fused walk-ancestor scratch decision")
