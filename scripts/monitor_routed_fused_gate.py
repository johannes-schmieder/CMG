...
REPO = "CMG"
WORKFLOW_NAME = "routed-fused-walk-scratch-v2.yml"
DECISION = Path(".ci/performance/routed-fused-walk-scratch-latest.json")
STATUS = Path(".ci/performance/routed-fused-walk-scratch-run-status.json")
MONITOR_WORKFLOW = Path(".github/workflows/monitor-routed-fused-gate.yml")
MONITOR_SCRIPT = Path("scripts/monitor_routed_fused_gate.py")
...
record = {
    "schema_version": 1,
    "monitor": "routed-fused-walk-scratch-v2",
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record routed fused gate status"])
for _ in range(8):
    ...
    raise RuntimeError("failed to push routed fused gate monitor result")
