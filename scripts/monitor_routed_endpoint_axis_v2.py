...
REPO = "CMG"
GATE_SHA = "ed6e20835bdb2e498050227980adb977f79dff1d"
DECISION = Path(".ci/performance/routed-endpoint-axis-sort-latest.json")
STATUS = Path(".ci/performance/routed-endpoint-axis-sort-v2-run-status.json")
WORKFLOW = Path(".github/workflows/monitor-routed-endpoint-axis-v2.yml")
SCRIPT = Path("scripts/monitor_routed_endpoint_axis_v2.py")
...
def request(path):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cmg-routed-endpoint-monitor",
        },
    )
...
record = {
    "schema_version": 1,
    "monitor": "routed-endpoint-axis-v2",
    "gate_sha": GATE_SHA,
...
for _ in range(120):
    ...
    if matches:
        ...
        for candidate in matches:
            if "endpoint-axis" in candidate.get("name", "").lower():
                latest = candidate
                break
        if latest is None:
            latest = matches[0]
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record routed endpoint-axis v2 status"])
for _ in range(10):
    ...
    raise RuntimeError("failed to push routed endpoint-axis monitor result")
