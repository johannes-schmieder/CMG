...
REPO = "CMG"
WORKFLOW_NAME = "confirm-dense-endpoint-axis-sort.yml"
GATE_SHA = "3ad2a691edbe1c420e2948baf2cbaf200b796306"
DECISION = Path(".ci/performance/dense-endpoint-axis-sort-latest.json")
STATUS = Path(".ci/performance/dense-endpoint-axis-confirmation-run-status.json")
WORKFLOW = Path(".github/workflows/monitor-dense-endpoint-axis-confirmation.yml")
SCRIPT = Path("scripts/monitor_dense_endpoint_axis_confirmation.py")
...
def request(path):
    request_object = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cmg-dense-confirmation-monitor",
        },
    )
...
record = {
    "schema_version": 1,
    "monitor": "dense-endpoint-axis-confirmation",
    "gate_sha": GATE_SHA,
...
for _ in range(240):
    payload = json_request(
        f"/repos/{OWNER}/{REPO}/actions/workflows/{WORKFLOW_NAME}/runs?per_page=10"
    )
    matches = [
        item
    ...
    for job in jobs.get("jobs", []):
        if job.get("conclusion") not in ("success", "skipped"):
            try:
                ...
                failed_logs[str(job["id"])] = (
                    f"log retrieval failed: {error!r}"
                )
    if failed_logs:
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record dense endpoint-axis confirmation status"])
for _ in range(12):
    ...
    raise RuntimeError("failed to push dense confirmation monitor result")
