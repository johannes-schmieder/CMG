...
TEMP_BENCH = Path("src/bin/split-conductance-gate.rs")
WORKFLOW = Path(".github/workflows/split-conductance-branch.yml")
SCRIPT = Path("scripts/split_conductance_branch_gate.py")
RECORD = Path(".ci/performance/split-conductance-branch-latest.json")
...
def build(target):
    ...
    run(["cargo", "build", "--release", "--bin", "split-conductance-gate"], env=env)
    run(
        [
            "cargo",
            "build",
    ...
    return {
        "split": target / "release" / "split-conductance-gate",
        "hierarchy": target / "release" / "hierarchy-build",
    }


def sample(binary, arguments, tag):
    time_path = Path(f"/tmp/cmg-conductance-{tag}.time")
    completed = run(
...
def compare(kind, baseline, candidate, arguments, name):
    ...
    stable = (
        ("case", "scale", "vertices", "edges", "levels", "parent_sets", "total_parent_vertices", "repetitions", "checksum")
        if kind == "split"
...
def update_documents(result):
    ...
    checkpoint = f'''### Branch-free conductance-pass checkpoint — 2026-08-24

- Separating conductance cut search from ancestor adjustment was **{decision}**.
...
'''
    ...
    if "### Branch-free conductance-pass checkpoint — 2026-08-24\n" not in plan:
    ...
    block = f'''## Branch-free conductance-pass gate
...
'''
    ...
    heading = "## Branch-free conductance-pass gate\n"
...
try:
    run(["cargo", "fmt", "--all"])
    baseline = build(Path("/tmp/cmg-conductance-baseline"))
    ...
    candidate = build(Path("/tmp/cmg-conductance-candidate"))
    specs = (
...
message = (
    ...
    else "perf: record branch-free conductance experiment"
)
...
for _ in range(5):
    ...
    raise RuntimeError("failed to push conductance-pass decision")
