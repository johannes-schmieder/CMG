...
def timed_sample(binary, arguments, tag):
    time_path = Path(f"/tmp/cmg-trim-buffer-{tag}.time")
    completed = run(
        [
            "/usr/bin/time",
            "-v",
...
def compare_case(baseline, candidate, arguments, name, metadata_keys):
    ...
    for key in metric_keys:
        ...
        if baseline_value != 0:
            ...
            ratio = float("inf")
        result[f"candidate_over_baseline_{key}"] = ratio
    return result
...
result = {
    "schema_version": 1,
    "experiment": "trimmed-compact-edge-build-buffer",
...
try:
    baseline = build(Path("/tmp/cmg-trim-buffer-baseline"))
    run(["python", "scripts/compact_edge_build_buffer_trim.py"])
    run(["cargo", "fmt", "--all"])
    ...
    candidate = build(Path("/tmp/cmg-trim-buffer-candidate"))
    ...
    result["acceptance_limits"] = {
        ...
        "hierarchy_additional_peak_geometric_mean_max": 0.98,
        "worst_exact_peak_allocation_max": 1.005,
    ...
    result["accepted"] = (
        ...
        and result["hierarchy_additional_peak_geometric_mean_ratio"] <= 0.98
        and result["worst_exact_peak_allocation_ratio"] <= 1.005
        and result["worst_retained_allocation_ratio"] <= 1.001
        and result["max_post_drop_delta_bytes"] == 0
    )
    result["decision_reason"] = (
        "full qualification passed; capacity trimming preserves graph gains and removes hierarchy over-retention"
        if result["accepted"]
        else "qualification passed but timing or exact hierarchy allocation missed the retention gate"
    )
...
retained_ratio = result.get("hierarchy_retained_geometric_mean_ratio")
evidence = ""
if None not in (
    graph_time_ratio,
    graph_rss_ratio,
    ...
    evidence = (
        ...
        f"hierarchy time/exact peak/retained: {hierarchy_time_ratio:.3f}/"
        f"{exact_peak_ratio:.3f}/{retained_ratio:.3f}."
    )
checkpoint = "\n".join(
    [
...
if "### Trimmed compact graph-buffer checkpoint — 2026-08-22" not in plan:
    plan = plan.replace(marker, checkpoint + marker, 1)
plan_path.write_text(plan)

for staging in (
    "scripts/compact_edge_build_buffer_trim.py",
    "scripts/compact_edge_build_buffer_trim_gate.py",
    ".github/workflows/compact-edge-build-buffer-trim.yml",
):
...
run(["git", "add", "-A"])
message = (
    "perf: compact and trim graph construction buffer"
    if result["accepted"]
    else "perf: record trimmed graph buffer experiment"
)
