...
new_builder = '''    fn from_sorted_raw_edges(
        ...
        // the front of the compact input buffer so graph construction never
        // allocates a second full-capacity retained-edge vector.
...
if text == original:
    raise SystemExit("compact graph-build buffer patch made no changes")
if "mut raw: Vec<Edge>" not in text:
    raise SystemExit("compact sorted edge buffer is missing")
if "Result<Vec<Edge>, CmgError>" not in text:
    raise SystemExit("compact validation buffer is missing")
if "let mut canonical" in text:
    raise SystemExit("second canonical edge allocation remains")
