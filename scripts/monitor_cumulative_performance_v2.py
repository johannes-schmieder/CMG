...
REPO = "CMG"
WORKFLOW_NAME = "refresh-cumulative-performance-v2.yml"
DECISION = Path(".ci/performance/cumulative-latest.json")
STATUS = Path(".ci/performance/cumulative-v2-run-status.json")
WORKFLOW = Path(".github/workflows/monitor-cumulative-performance-v2.yml")
SCRIPT = Path("scripts/monitor_cumulative_performance_v2.py")
...
def request(path):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cmg-cumulative-v2-monitor",
        },
    )
...
record = {
    "schema_version": 1,
    "monitor": "cumulative-performance-refresh-v2",
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record cumulative v2 status"])
for _ in range(12):
    ...
    raise RuntimeError("failed to push cumulative v2 monitor result")
