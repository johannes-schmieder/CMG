"""Qualify and conditionally retain the prepared parallel PCG solver."""
...
TOUCHED = ['src/lib.rs', 'tests/parallel.rs', 'benchmarks/Cargo.lock']
NEW_FILES = [
    'src/parallel_solver.rs',
    'benchmarks/src/bin/prepared-solver-auto.rs',
]
command_output: list[str] = []

RUST_BENCHMARK = r'''use std::hint::black_box;
use std::time::Instant;

use cmg::{
    CmgOptions, Laplacian, ParallelPcgExecution, ParallelPcgPolicy,
    ParallelPcgSolver, ParallelOptions, PcgOptions, PcgResult, PcgWorkspace,
...
fn main() {
    ...
    let rhs_count = arguments
        .next()
        .map(|value| value.parse::<usize>().expect("RHS count must be an integer"))
        .unwrap_or(4)
        .max(1);
    ...
    println!(
        "{{\"case\":\"{case}\",\"scale\":{scale},\"vertices\":{},\"edges\":{},\"rhs_count\":{rhs_count},\"threads\":{},\"levels\":{},\"operators\":{},\"execution\":\"{}\",\"concurrency\":{},\"serial_ns\":{serial_ns},\"across_rhs_ns\":{across_ns},\"planned_ns\":{planned_ns},\"auto_ns\":{auto_ns},\"best_explicit_ns\":{best_explicit_ns},\"auto_over_best\":{:.17e},\"serial_over_best\":{:.17e},\"across_over_best\":{:.17e},\"planned_over_best\":{:.17e},\"build_ns\":{build_ns},\"plan_bytes\":{},\"workspace_bytes_each\":{},\"workspace_pool_bytes\":{},\"retained_workspace_bytes\":{},\"max_across_difference\":{across_difference:.17e},\"max_planned_difference\":{planned_difference:.17e},\"max_auto_difference\":{auto_difference:.17e}}}",
...
def restore() -> None:
    run(['git', 'checkout', 'HEAD', '--', *TOUCHED], timeout=120)
    for name in NEW_FILES:
        Path(name).unlink(missing_ok=True)


def sample(binary: Path, arguments: list[Any], tag: str) -> dict[str, Any]:
    time_path = Path(f'/tmp/cmg-prepared-{tag}.time')
    completed = run(
        [
            '/usr/bin/time',
            '-v',
...
try:
    run(['python', str(CANDIDATE)])
    benchmark_path = Path(NEW_FILES[1])
    benchmark_path.parent.mkdir(parents=True, exist_ok=True)
    benchmark_path.write_text(RUST_BENCHMARK)
    run(['cargo', 'fmt', '--all'])
    ...
    run([
        'cargo', 'build', '--manifest-path', 'benchmarks/Cargo.toml', '--bin', 'prepared-solver-auto', '--release'
    ])
    result['validation'] = 'success'

    binary = ROOT / 'benchmarks/target/release/prepared-solver-auto'
    specs: list[tuple[str, list[Any]]] = []
    for rhs_count in (1, 2, 8):
        specs.extend(
            [
    ...
    for name, arguments in specs:
        ...
        ratios.append(observation['auto_over_best'])
    ...
    result['maximum_auto_over_best'] = max(ratios)
    ...
    result['decision_reason'] = (
        'full qualification passed; prepared solver reproduced the observed best-strategy router with reusable bounded workspaces'
        if result['accepted']
        else 'qualification passed but timing, numerical, or workspace-retention gates were not met'
    )
...
if PLAN.exists():
    ...
    checkpoint = f'''### Prepared parallel solver checkpoint — 2026-08-23

- The memory-aware prepared solver candidate was **{status}**.
...
'''
    ...
    if '### Prepared parallel solver checkpoint — 2026-08-23' not in text:
...
message = (
    ...
    if result['accepted']
    else 'perf: record prepared parallel solver experiment'
)
