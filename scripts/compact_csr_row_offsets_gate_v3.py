...
text = text.replace(
    ...
    'WORKFLOW = Path(".github/workflows/compact-csr-row-offsets-v3.yml")',
    1,
)
text = text.replace(
    'SCRIPT = Path("scripts/compact_csr_row_offsets_gate.py")',
    'SCRIPT = Path("scripts/compact_csr_row_offsets_gate_v3.py")',
    1,
)
...
for marker in (
    "compact_csr_row_offsets_gate_v3.py",
    "compact-csr-row-offsets-v3.yml",
    'source_patch = r"""ROW_OFFSETS_INSERT_MARKER',
):
    if marker not in text:
        raise SystemExit(f"syntax-safe compact CSR gate missing marker: {marker}")
