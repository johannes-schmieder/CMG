...
ROOT = Path.cwd()
TARGET_SCRIPT = Path("scripts/zipped_ancestor_updates_gate.py")
TARGET_WORKFLOW = Path(".github/workflows/zipped-ancestor-updates.yml")
RECOVERY_SCRIPT = Path("scripts/recover_zipped_ancestor_updates.py")
RECOVERY_WORKFLOW = Path(".github/workflows/recover-zipped-ancestor-updates.yml")
RECOVERY_RECORD = Path(
    ".ci/performance/zipped-ancestor-updates-recovery.json"
)
...
record = {
    "schema_version": 1,
    "recovery": "zipped-ancestor-updates",
...
# The target may have committed and pushed a numerical decision. Synchronize
# before writing the recovery checkpoint and cleaning stale one-shot files.
...
run(["git", "add", "-A"])
commit = run(
    ["git", "commit", "-m", "ci: record zipped ancestor gate recovery"],
    check=False,
)
if commit.returncode == 0:
    for _ in range(5):
        ...
        raise SystemExit("failed to push zipped ancestor recovery")
