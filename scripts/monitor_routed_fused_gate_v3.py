...
REPO = "CMG"
WORKFLOW_NAME = "routed-fused-walk-scratch-v3.yml"
DECISION = Path(".ci/performance/routed-fused-walk-scratch-latest.json")
STATUS = Path(".ci/performance/routed-fused-walk-scratch-v3-run-status.json")
MONITOR_WORKFLOW = Path(".github/workflows/monitor-routed-fused-gate-v3.yml")
MONITOR_SCRIPT = Path("scripts/monitor_routed_fused_gate_v3.py")
...
record = {
    "schema_version": 1,
    "monitor": "routed-fused-walk-scratch-v3",
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record routed fused v3 gate status"])
for _ in range(8):
    ...
    raise RuntimeError("failed to push routed fused v3 monitor result")
