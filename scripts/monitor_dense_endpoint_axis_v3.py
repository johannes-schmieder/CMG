...
REPO = "CMG"
GATE_SHA = "da4dd6bcf8515389f647a7a7f1b9c5d0bd78c54f"
WORKFLOW_NAME = "dense-endpoint-axis-sort-v3.yml"
DECISION = Path(".ci/performance/dense-endpoint-axis-sort-latest.json")
STATUS = Path(".ci/performance/dense-endpoint-axis-sort-v3-run-status.json")
MONITOR_WORKFLOW = Path(".github/workflows/monitor-dense-endpoint-axis-v3.yml")
MONITOR_SCRIPT = Path("scripts/monitor_dense_endpoint_axis_v3.py")
...
def request(path):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cmg-dense-endpoint-v3-monitor",
        },
    )
...
record = {
    "schema_version": 1,
    "monitor": "dense-endpoint-axis-v3",
    "gate_sha": GATE_SHA,
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record dense endpoint-axis v3 status"])
for _ in range(10):
    ...
    raise RuntimeError("failed to push dense endpoint-axis v3 monitor result")
