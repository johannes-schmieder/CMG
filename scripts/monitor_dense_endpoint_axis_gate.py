...
REPO = "CMG"
GATE_SHA = "b4a434490a9b352a90443394bd28878ccfa74088"
WORKFLOW_NAME = "dense-endpoint-axis-sort.yml"
DECISION = Path(".ci/performance/dense-endpoint-axis-sort-latest.json")
STATUS = Path(".ci/performance/dense-endpoint-axis-sort-run-status.json")
MONITOR_WORKFLOW = Path(".github/workflows/monitor-dense-endpoint-axis-gate.yml")
MONITOR_SCRIPT = Path("scripts/monitor_dense_endpoint_axis_gate.py")
...
def request(path):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cmg-dense-endpoint-monitor",
        },
    )
...
record = {
    "schema_version": 1,
    "monitor": "dense-endpoint-axis-sort",
    "gate_sha": GATE_SHA,
...
run(["git", "add", "-A"])
run(["git", "commit", "-m", "ci: record dense endpoint-axis gate status"])
for _ in range(10):
    ...
    raise RuntimeError("failed to push dense endpoint-axis monitor result")
