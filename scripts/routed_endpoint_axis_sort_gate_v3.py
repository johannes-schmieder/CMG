...
text = text.replace(
    ...
    "routed_endpoint_axis_sort_gate_v3.py",
)
text = text.replace(
    "routed-endpoint-axis-sort.yml",
    "routed-endpoint-axis-sort-v3.yml",
)
...
SCRIPT.unlink(missing_ok=True)
for stale in (
    ".github/workflows/routed-endpoint-axis-sort.yml",
    "scripts/routed_endpoint_axis_sort_gate.py",
    ".github/workflows/routed-endpoint-axis-sort-v2.yml",
    "scripts/routed_endpoint_axis_sort_gate_v2.py",
):
    Path(stale).unlink(missing_ok=True)
"""
if text.count(cleanup_marker) != 1:
    raise SystemExit("historical routed cleanup marker changed unexpectedly")
...
for marker in required:
    if marker not in text:
        raise SystemExit(f"syntax-safe routed endpoint gate missing marker: {marker}")
