...
'''
    ...
    replacements = {
        ...
        'let p_ap = dot(&workspace.direction, &workspace.matvec);':
            'let p_ap = dot_with_executor(&workspace.direction, &workspace.matvec, executor);',
        'let rho_next = dot(&workspace.residual, &workspace.preconditioned);':
            'let rho_next = dot_with_executor(\n            &workspace.residual,\n            &workspace.preconditioned,\n            executor,\n        );',
    }
...
def compare_case(baseline, candidate, arguments):
    ...
    exact_keys = (
        'case', 'scale', 'vertices', 'edges', 'input_edges', 'levels',
...
'''
if '### Deterministic fixed-chunk dot checkpoint — 2026-08-23' not in plan:
    plan = plan.replace(marker, checkpoint + marker, 1)
plan_path.write_text(plan)

status_path = Path('PERFORMANCE_STATUS.md')
status_text = status_path.read_text().rstrip()
if '## Deterministic fixed-chunk dot gate' not in status_text:
    status_text += (
...
Path('.github/workflows/fixed-chunk-dot.yml').unlink(missing_ok=True)
Path('scripts/fixed_chunk_dot_gate.py').unlink(missing_ok=True)
