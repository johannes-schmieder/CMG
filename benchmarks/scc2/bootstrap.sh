...
python3 -m compileall -q "$code_root/benchmarks/scc2" \
    > "$run_root/logs/python-compileall.log" 2>&1
python3 -m unittest discover -s "$code_root/benchmarks/scc2/tests" -v \
    > "$run_root/logs/python-tests.log" 2>&1
for kind in smoke baseline routing reuse numa memory accuracy batch matched-edge; do
    python3 - "$run_root/manifests/tasks/$kind.jsonl" "$code_root/benchmarks/scc2/schemas/task.schema.json" <<'PY'
import json
import sys
import jsonschema
...
python3 "$code_root/benchmarks/scc2/tasks/generate_tasks.py" smoke "$run_root/work/tasks-smoke-roundtrip.jsonl" > "$run_root/logs/task-generator.log" 2>&1
